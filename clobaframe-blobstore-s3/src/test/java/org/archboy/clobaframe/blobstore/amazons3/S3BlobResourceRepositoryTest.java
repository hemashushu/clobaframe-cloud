...
public class S3BlobResourceRepositoryTest {
	...
	public void testPut() throws IOException {
		...
		assertDateEquals(now1, blobResourceInfo1.getLastModified(), 60);
...
//		Date tenMinsAgo = calendar.getTime();
		...
		assertDateEquals(now2, blobResourceInfo2.getLastModified(), 60);
		...
		assertEquals("100", metaByBlob1.get("amount")); // amazon s3 only allow String values.
		assertEquals("99.1", metaByBlob1.get("price"));
		assertEquals(oneHourAgo.toString(), (String)metaByBlob1.get("update"));
	...
	private static void assertDateEquals(Date expected, Date actual, int thresholdSeconds){
		if (expected == null && actual == null){
			...
			assertTrue(String.format("expected %s, but actual is %s.", expected, actual),
					Math.abs(expected.getTime() - actual.getTime()) < thresholdSeconds * 1000L);
		}
	}
}
